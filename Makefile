...
# C sources
C_SOURCES += src/main.c src/menu.c src/dmforth.c src/zforth.c src/alpha.c src/fmenu.c
...
LDFLAGS = $(CPUFLAGS) -T$(LDSCRIPT) $(LIBDIR) $(LIBS) -Wl,-Map=$(BUILD_DIR)/$(TARGET).map,--cref -Wl,--gc-sections \
  -Wl,--wrap=_malloc_r --specs=nosys.specs
...
src/zcore.h: forth/core.zf forth/math.zf forth/dm.zf
	cat $^ > all.zf
	xxd -i all.zf $@
	rm all.zf


$(BUILD_DIR)/dmforth.o: src/dmforth.c src/zcore.h
...
#######################################
clean:
	-rm -fR .dep $(BUILD_DIR)/*.o $(BUILD_DIR)/*.lst $(SRC)/zcore.h
